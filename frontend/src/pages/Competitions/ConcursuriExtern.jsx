...
const ConcursuriExtern = () => {
    ...
    // sus, lângă alte constante
    const BELT_GRADES = [
        "Albă",
        "Albă cu dungă galbenă",
        "Galbenă",
        "Galbenă cu dungă verde",
        "Verde",
        "Verde cu dungă albastră",
        "Albastră",
        "Albastră cu dungă roșie",
        "Roșie",
        "Roșie cu dungă neagră",
        "Neagră",
    ];
    ...
    const handleSubmit = async (e) => {
        ...
        if (res.ok) {
            ...
            setFormData({nume: "", dataNasterii: "", categorieVarsta: "", gradCentura: "", greutate: "", gen: ""});
            setSelectedProbes([]);
            setGender("");
            setFormularDeschis(null); // închide cardul
        } else {
    ...
    return (
        <>
            <Navbar/>
            <div className="concursuri-container concursuri-extern">
                <h2 className="concursuri-title">Concursuri viitoare</h2>

                <div className="extern-grid">
                    {Array.isArray(concursuri) && concursuri.map((conc, index) => (
                        <div key={index} className="concurs-card">
                            ...
                            {formularDeschis !== index ? (
                                <button className="btn-inscriere" onClick={() => setFormularDeschis(index)}>
                                    Înscrie sportiv
                                </button>
                            ) : (
                                <form onSubmit={handleSubmit} className="form-inscriere form-inscriere--extern">
                                    <input
                                        name="nume"
                                    ...
                                    <Select
                                        ...
                                        className="react-select-container extern-select"
                                        classNamePrefix="react-select"
                                        placeholder="Alege probele..."
                                    />

                                    <div className="extern-actions">
                                        <button className="btn-inscriere" type="submit">Trimite înscrierea</button>
                                        <button className="btn-inscriere btn-cancel" type="button"
                                                onClick={() => setFormularDeschis(null)}>
                                            Anulează
                                        </button>
                                    </div>
                                </form>
